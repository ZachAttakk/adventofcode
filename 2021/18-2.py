...
def main():
    ...
    result_magnitude: int = 0

    # HERE WE GO
    current_number: str = data[0]

    for i, main_num in enumerate(data):
        for j, sub_num in enumerate(data):
            if i == j:
                continue  # don't want to add something to itself

            current_number = f"[{main_num},{sub_num}]"
            ...
            current_magnitude: int = int(calc_magnitude(current_number))
            if current_magnitude > result_magnitude:
                result_magnitude = current_magnitude

    printGood(result_magnitude)
