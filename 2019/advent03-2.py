...
final_dist = 0
found = False
for _idx_a, _pos_a in enumerate(array[0]):
  #print("Looking for ", _pos)
  if found: break
  percent = ((_idx_a+1)/len(array[0]))*100
  if int(percent) != last_perc: 
        print(int(percent))
        last_perc = int(percent)

  for _idx_b, _pos_b in enumerate(array[1]):
    if _pos_a == _pos_b:
      print("Intersecton found: ", _pos_a)
      final_dist = _idx_a + _idx_b + 2
      found = True
      break
